...
| `--n8n-base-url <url>` | No | Self-hosted n8n instance URL (admin only) | Platform default |
| `--n8n-api-key <key>` | No | Self-hosted n8n API key (admin only) | Platform default |
...
1. Calls the Codika platform API to create the organization
2. The authenticated user becomes the organization owner
3. Initializes free plan credits and n8n secrets
4. If self-hosted n8n flags are provided, validates the credentials before creating
