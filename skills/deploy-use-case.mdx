...
The `deployments` map in `project.json` is used by `codika-helper publish` to promote a deployment to production.
...
<Note>
Data ingestion workflows live in a separate `data-ingestion/` folder and are deployed independently via `codika-helper deploy process-data-ingestion`. They have their own version line and do not trigger "update available" notifications.
</Note>
