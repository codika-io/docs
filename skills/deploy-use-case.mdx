...
The `deployments` map in `project.json` is used by `codika publish` to promote a deployment to production.
...
# Minor version
codika deploy use-case . --minor

# Major version
codika deploy use-case . --major

# Explicit API version
codika deploy use-case . --version 2.0
