...
### Redeploying without versioning

Sometimes you need to change deployment parameters (e.g., company name, language, timezone) without creating a new template version. The `codika redeploy` command handles this:
...
codika redeploy --param LANGUAGE=fr

# Retry a failed deployment
codika redeploy --environment dev

# Update prod parameters
codika redeploy --environment prod --param COMPANY_NAME="New Corp" --force
```

See the [`codika redeploy` reference](/operations/redeploy-use-case) for full details.
