...
| `workflows/*.json` | n8n workflow JSON files. Each file listed in `WORKFLOW_FILES` is base64-encoded and sent to the platform. |
...
When you deploy a use case, the platform creates four Firestore entities:

```
Use case folder
  → Process (public listing, discoverable)
    → ProcessDeploymentTemplate (immutable version snapshot)
      → ProcessDeploymentInstance (user-specific deployment with n8n workflow IDs)
        → ProcessInstance (user-facing installation with settings and activation state)
```

See [Processes](/concepts/processes) for full details on these entities.
