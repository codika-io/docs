...
description: "Three ways to authenticate — API keys for dashboards and CLI, webhook signatures for external platforms"
---

## Authentication methods

Codika supports three authentication methods on its public API:

| | Instance Key (`ck_`) | Organization Key (`cko_`) | Webhook Signature (HMAC) |
|---|---|---|---|
| **Created by** | Auto-generated per process instance | You create in the Codika dashboard | External platform (Resend, Stripe, etc.) |
| **Scope** | One specific process instance | Any instance in the organization | One specific workflow endpoint |
| **Best for** | Custom dashboards, apps | CLI, agents, CI/CD | External platforms that sign outbound webhooks |
| **Header** | `X-API-Key` | `X-Process-Manager-Key` | `webhook-signature` + `webhook-id` + `webhook-timestamp` |
| **Secret on the wire** | Yes | Yes | No (only a signature) |
