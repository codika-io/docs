...
The template includes three workflows demonstrating different trigger types (HTTP, schedule, sub-workflow), the mandatory Codika node pattern, and placeholder usage. Dependencies are installed automatically so the use case is immediately deployable.
...
```bash
codika-helper publish <templateId>
```
...
This creates a production process instance. See [publish](/cli/publish) for visibility and sharing options.
