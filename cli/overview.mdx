...
The CLI communicates with these Cloud Function endpoints:

| Operation | Endpoint |
|-----------|----------|
| Deploy use case | `POST /deployProcessUseCase` |
| Deploy data ingestion | `POST /deployDataIngestion` |
| Create project | `POST /createProjectViaApiKey` |
| Get metadata documents | `GET /getMetadataDocumentsEndpoint` |
| Get execution details | `GET /getExecutionDetailsPublic` |
| Trigger workflow | `POST /triggerWebhookPublic` |
| Get execution status | `GET /getExecutionStatusPublic` |
| Verify API key | `POST /verifyApiKey` |

All authenticated requests use the `X-Process-Manager-Key` header with the API key value.
