...
2. Calls `/verifyApiKey` to validate the key (unless `--skip-verify`)
...
Calls `/verifyApiKey` for fresh data. Falls back to cached profile data if the network call fails.
